...
function organic_farm_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting('organic_farm_read_more_icon',array(
		'default'	=> 'fas fa-arrow-right',
		'sanitize_callback'	=> 'sanitize_text_field'
	));	
	$wp_customize->add_control(new Organic_Farm_Fontawesome_Icon_Chooser(
        $wp_customize,'organic_farm_read_more_icon',array(
		'label'	=> __('Read More Icon','organic-farm'),
		'transport' => 'refresh',
		'section'	=> 'organic_farm_archieve_post_layot',
		'setting'	=> 'organic_farm_read_more_icon',
		'type'		=> 'icon'
	)));
	...
	if ( class_exists( 'WooCommerce' ) ) { 
	    ...
	    $wp_customize->add_control('woocommerce_related_products_heading', array(
	        'label' => __('Related Products Heading', 'organic-farm'),
	        'section' => 'organic_farm_woocommerce_settings',
	        'type' => 'text',
	    ));
